...
app.layout = dash.html.Div([
    dbc.Row([
        dbc.Row(dbc.Card(dash.html.H1('Cryptocurrency Trading'),color='blue', style={'color': 'white','text-align': 'center'})),
        dbc.Col([


            dbc.Row( [
        dbc.Row(dash.html.H6(
...
def drop(value):
    ...
    df = df.sort_values('start')
    df.index = df.start
    fig = px.line(x=df.index, y=df['close']).add_traces(
        px.line(x=df.index, y=df['close']
                )
            .update_traces(marker_color="rgba(0,0,0,0)").data)
    fig.layout.height = 400
